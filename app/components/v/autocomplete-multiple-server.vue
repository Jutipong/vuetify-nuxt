...
watch(vModel, (newVal, oldVal) => {
    ...
    if (newIds.length === oldIds.length && newIds?.every(newId => oldIds?.includes(newId)))
        return

    const propsIds = modelValue?.map(id => id) ?? []

    if (newVal?.length === propsIds?.length && newIds?.every(newId => propsIds?.includes(newId)))
        return
...
watch(() => modelValue, async (newVal) => {
    ...
    if (newIds?.every(newId => vModelId?.includes(newId)))
        return
